...
{
	glPointSize(2.0); // Changer la taille des points selon vos préférences

	// Calculer le pas pour les points en fonction du nombre de points spécifié
	float step = 1.0 / (float)nb_points;
	float halfStep = step / 2.0;

	// Dessiner le carré en pointillés
	glBegin(GL_POINTS);
	// Dessiner les points en haut et en bas du carré
	for (float x = -0.5; x <= 0.5 + halfStep; x += step)
	{
		glVertex3f(x, 0.5, 0);
		glVertex3f(x, -0.5, 0);
	}
	// Dessiner les points sur les côtés du carré
	for (float y = -0.5; y <= 0.5; y += step)
	{
		glVertex3f(-0.5, y, 0);
		glVertex3f(0.5, y, 0);
	}
